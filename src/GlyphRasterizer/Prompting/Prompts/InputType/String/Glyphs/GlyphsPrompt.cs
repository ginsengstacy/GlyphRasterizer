...
{
    private GlyphTypeface? _currentTypeface;

    protected override string Message => string.Format(PromptMessages.Glyphs_FormatString, Environment.NewLine);

    protected override Func<GlyphParseContext> GetInput => () => new(ConsoleHelpers.ReadLineSafe(), _currentTypeface);
    ...
    public void SetSessionContext(SessionContext context) => _currentTypeface = context.GlyphTypeface;
}